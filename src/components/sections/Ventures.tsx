          ...
          {ventures.map((venture, i) => (
            <Reveal key={venture.name} direction="up" delay={i * 0.08}>
              <TiltCard className="group">
                <Link
                  href={venture.href.startsWith("[TODO") ? "#" : venture.href}
                  target={venture.href.startsWith("http") ? "_blank" : undefined}
                  rel={venture.href.startsWith("http") ? "noopener" : undefined}
                  className="block p-7"
                >
                  ...
                  {venture.href.startsWith("[TODO") && (
                    <p className="mt-3 text-xs text-muted/60">{venture.href}</p>
                  )}
                </Link>
              </TiltCard>
            </Reveal>
